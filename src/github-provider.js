...
exports.fetchGitHubResumeData = fetchGitHubResumeData;
/**
...
class GitHubAIGenerator {
    ...
    detectProjectType(name, topics) {
        ...
        if (lowerName.includes('pixel') || lowerName.includes('word'))
            return 'game';
     ...
     */
    generateAchievement(stars, forks) {
        if (stars === 0 && forks === 0) {
     ...
     */
    generateDescription(repoName, description, language, topics, readmeContent) {
        var _a;
        const projectType = this.detectProjectType(repoName, topics);
        const projectConcept = ((_a = this.projectTypes[projectType]) === null || _a === void 0 ? void 0 : _a[0]) || 'software solution';
        // Extract keywords from README if available
        let readmeKeywords = [];
        if (readmeContent) {
            const words = readmeContent.split(/\s+/).filter(w => w.length > 4);
            const importantWords = words.filter(w => ['performance', 'fast', 'efficient', 'modern', 'clean', 'modular',
     ...
     */
    generateSkills(repos, readmeContent) {
...
function fetchGitHubResumeData(input) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const experience = topRepos.map(repo => {
            const bullets = [];
            ...
            const smartDescription = aiGenerator.generateDescription(repo.name, repo.description, repo.language, repo.topics || [], readmeContent || null);
